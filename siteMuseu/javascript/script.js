            ...
            for (k = 0; k < s.rules.length; k++) {
                ...
                for (l = 0; l < values.length; l++) {
                    ...
                    if (lastStop !== null && maxOffset > 0) {
                        last = '<rect ' + (isLeft ? 'x="' + maxOffset + '" y="0"' : 'x="0" y="' + maxOffset + '"') + ' width="100%" height="100%" style="fill:' + lastStop.color + ';opacity:' + lastStop.opacity + ';"/>';
...
jQuery(function ($) {
    $("ul.art-hmenu ul li").hover(function () {
        $(this).prev().children("a").addClass("art-hmenu-before-hovered");
    }, function () {
        $(this).prev().children("a").removeClass("art-hmenu-before-hovered");
    });
});
...
var ThemeLightbox = (function ($) {
    ...
    return (function () {
        ...
        function show(src) {
            var closeBtn = $('<div class="close"><div class="cw"> </div><div class="ccw"> </div><div class="close-alt">&#10007;</div></div>').click(close);

            var imgContainer = $('.art-lightbox-wrapper');
            if (imgContainer.length === 0) {
                imgContainer = $('<div class="art-lightbox-wrapper">').css('line-height', $(window).height() + "px").appendTo($("body"));
            }
        ...
        function showError(enable) {
            if (enable) {
                $(".art-lightbox-wrapper").append($('<div class="lightbox-error">Não é possível carregar o conteúdo pedido.<br/>Por favor tente mais tarde.</div>').css({
...
(function ($) {
            ...
            if (initialSize && parseInt(initialSize.width, 10) !== 0) {
                ...
                if (motion === "fade") {
                    $.each(element.children(), function (i) {
                        $(this).css("background-position", getCssPositions(slides[i].positions, {
                            x: 0,
                            y: 0
                        }));
                    });
    ...
    var Slider = function (element, settings) {
        ...
        this.move = function (direction, next) {
            ...
            if ($.support.transition) {
                ...
                element.one($.support.transition.event, function () {
                    nextItem.removeClass(slider.settings.direction).removeClass(innerDirection).addClass("active");
        ...
        if (this.settings.hover) {
            var slider = this;
            element.add(this.settings.navigator).add(element.siblings(".art-shapes")).hover(function () {
...
function findAll() {
    ...
    $("section#centralSection").load("acervo/pesquisar.html"); //add
            ...
            $('body').on('click', 'img.imagemBanco', function () {
                ...
                if (i == false) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        data: {},
                        url: "http://143.107.183.133:8080/memoriavirtualWebService/rest/buscar/" + id,
                        headers: {
                            'Authorization': 'bXZpcnR1YWw6bXZpcnR1YWw=',
                            'Accept': 'application/json'
                        },
